...
def _require_admin_access():
    ...
    if not hasattr(actor, "user_id"):
        return access_control, None, actor
...
def clear_btcusdt_cache():
    ...
    try:
        ...
        return error_response(str(exc), status_code=500, code="BTCUSDT_CLEAR_FAILED")

    SystemController.record_event(
        scope="market_data",
        action="BTCUSDT cache cleared",
        actor=actor,
        message=f"Cleared {cleared} BTCUSDT kline rows",
    )
    return ok_response(
        {
            "data": {
