...
export function ExperimentDetailView() {
  ...
  return <BaseView title="Experiment Detail" description="Model-search dashboard for ranking, diagnosing, and exporting experiment results." actions={actions}>
    <div className="space-y-4"><span className="sr-only">Configuration</span>
      ...
      <Card><CardHeader><CardTitle>Indicators</CardTitle></CardHeader><CardContent>{Array.isArray(readableIndicators.selected) && readableIndicators.selected.length ? <div className="space-y-3">{readableIndicators.selected.map((name) => <div className="rounded border p-3" key={String(name)}><p className="font-medium">{String(name)}</p><ParameterList params={((readableIndicators.parameters as AnyRecord | undefined)?.[String(name)] as AnyRecord | undefined) ?? {}} /></div>)}</div> : <p className="text-sm text-muted-foreground">No indicators selected.</p>}</CardContent></Card>
