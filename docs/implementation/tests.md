...
```text
Backend: 364 passed
Frontend Jest: 24 test suites passed, 113 tests passed
...
## `backend/tests/test_fyp_integration_journey.py`

Explanation: Proves the core FYP prototype journey crosses backend feature boundaries. It registers users, promotes a moderator, moves a Blueprint from draft to pending to approved, creates an experiment through the HTTP endpoint, verifies the queued job response and compiled snapshots, marks the run completed with model/backtest metrics, then verifies that the model appears in rankings and the completed experiment/model appear in Public Hub.

Pseudocode:

```text
patch queue service with deterministic fake queue
register owner, moderator, viewer
seed draft Blueprint for owner
POST request approval as owner
POST approve as moderator
POST create experiment as owner
assert queued response, job ID, compiled snapshots, parameter hash
simulate completed experiment with model metrics and internal backtest log
GET model rankings as viewer and expect model/backtest metrics
GET Public Hub models and experiments as viewer and expect public artifacts
```
